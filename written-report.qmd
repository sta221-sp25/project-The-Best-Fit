...
These graphs shows a weak, positive relationship betweek article length and the log transformed number of shares. Due to the skew, we can apply the log transform to the article length. This shows a more even distribution, with no clear relationship.
...
newsdf |>
  ggplot(aes(x = title_sentiment_category, y = mean(shares))) + 
  geom_bar(stat = "identity") + 
  facet_wrap(~ data_channel)
```

From this visualization, we found that generally regardless of data channel type, articles with a mainly neutral tone experienced the greatest mean number of shares. Quantitatively, it appears as though social media experienced the least number of mean shares overall, regardless of tone,
