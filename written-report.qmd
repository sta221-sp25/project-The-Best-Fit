...
$$
\begin{aligned}\log\left( \frac{\pi}{1 - \pi} \right) =\ & \beta_0 + \beta_1\ \text{kw_avg_avg} + \beta_2\ \log(\text{n_tokens_content} + 10^{-4}) + \beta_3\ \text{global_subjectivity} + \beta_4\ \text{title_sentiment_polarity} \\& + \left( \sum_{i=1}^{4} \beta_{4+i}\ \mathbf{1}\{\text{data_channel} = \text{level}_i\} \right) \\& + \left( \sum_{j=1}^{6} \beta_{8+j}\ \mathbf{1}\{\text{day_published} = \text{level}_j\} \right)\end{aligned}
$$
...
Limitations in our model include the following: 

We assumed that all our observations/articles were independent from one another, however, this may not be the case as our data was collected across a mutli-year time scale, so thus outside events that we did not account for could have impacted the number of shares a given article received and thus our response variable. Another limitation was that not all the metrics we used are fully interpretable, such as “keyword average average”, as these metrics were derived by previous researchers, and we do not know how these metrics were derived. Finally, our research only focused on one news website, and thus we’re unsure if our results/model could be applied to all articles entirely. 

In the future, it would be helpful to control for time period in our model (to prevent issues with independence) and compare articles in similar groups with one another. Our final AUC curve also insinuates that our model was not the best fit to our data, and thus it could be helpful for us to consider other predictors in the model.
