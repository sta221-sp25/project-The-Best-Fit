...
news_data <- news_data |>
  mutate (
    day_published = case_when(
    news_data$weekday_is_monday == 1 ~ "Monday",
    news_data$weekday_is_tuesday == 1 ~ "Tuesday",
    news_data$weekday_is_wednesday == 1 ~ "Wednesday",
  news_data$weekday_is_thursday == 1 ~ "Thursday",
  news_data$weekday_is_friday == 1 ~ "Friday",
  news_data$weekday_is_saturday == 1 ~ "Saturday",
  news_data$weekday_is_sunday == 1 ~ "Sunday",
  )
)

news_data <- news_data |>
  mutate(
    data_channel = case_when (
  news_data$data_channel_is_entertainment == 1 ~ "Entertainment",
  news_data$data_channel_is_bus == 1 ~ "Business",
  news_data$data_channel_is_socmed == 1 ~ "Social Media",
  news_data$data_channel_is_tech == 1 ~ "Technology",
  news_data$data_channel_is_world == 1 ~ "World"
    )
  )
...
results_table <- data.frame(
  Metric = c("R^2", "Adjusted R-squared", "RMSE", "F-statistic", "p-value"),
  Value = c(
