...
data_channel - a catagorical variable merged from Data_channel_is_entertainment, data_channel_is_bus, data_channel_is_socmed, data_channel_is_tech,  and data_channel_is_world dennoting the tag on the article.

day_published- Weekday_is_monday, weekday_is_tuesday, weekday_is_wednesday, weekday_is_thursday, weekday_is_friday,weekday_is_saturday, weekday_is_sunday  – These are indicator variables for what day the article was published, which we can merge into a categorical variable of Day_published
...
Examining the rate of positive words in an article, we see

Finally, our initial EDA of title polarity found a massive frequency spike at 0 frequency, which might correspond to failed measurements or the vast majority of our articles not presenting signifigant title polarity. As such, we will discard title polarity fron our model.
